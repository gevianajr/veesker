...
describe("SqlEditor", () => {
  it("mounts a CodeMirror editor", async () => {
    const { container } = render(SqlEditor, {
      props: { value: "SELECT 1 FROM DUAL", onChange: noop, onRunCursor: noop, onRunAll: noop },
    });
  ...
  it("displays the initial value", async () => {
    const { container } = render(SqlEditor, {
      props: { value: "SELECT * FROM dual", onChange: noop, onRunCursor: noop, onRunAll: noop },
    });
    await new Promise((r) => setTimeout(r, 0));
  ...
  it.skip("calls onRunCursor when Mod-Enter is triggered", async () => {
    ...
    const { container } = render(SqlEditor, {
      props: { value: "SELECT 1", onChange: noop, onRunCursor, onRunAll: noop },
    });
    await new Promise((r) => setTimeout(r, 0));
